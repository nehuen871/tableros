...
router.post('/edit', (req, res) => {
  ...
  let {tableros_idtableros,roles_idroles} = req.body;
  const query = `UPDATE tableros_has_roles SET tableros_idtableros=?,roles_idroles=? WHERE tableros_idtableros = ?;`;
  mysqlConnection.query(query,[tableros_idtableros,roles_idroles], (err, rows, fields) => {
    if(!err) {
      if(rows.length == 0){
        res.json(0);
...
router.post('/delete', (req, res) => {
  ...
  const query = `DELETE from tableros_has_roles WHERE tableros_idtableros = ?;`;
